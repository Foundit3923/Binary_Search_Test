...
with open(op.join(CWD,ranges), 'r') as f:
...
with open(op.join(CWD,index), 'r') as f:
    hex_val = -1
    size = 0
    offsets = []
    for line in f.readlines():
...
for key in index_arr.keys():
    output_dict['compliant'] = index_arr[key]['offsets'].intersection(range_sets)
    output_dict['different'] = index_arr[key]['offsets'].difference(range_sets)
print("compliant offsets finished!")


print(f'Store in {output_file}')
json.dump(output_dict,open(output_file, 'w'), cls=SetEncoder)
